...
支持两种策略：
1. 全局剪枝率：在所有层中总共保留 X% 的专家
2. 每层剪枝率：每层都保留 X% 的专家（推荐）

使用二分查找自动找到合适的 alpha 值
"""
...
def find_alpha_for_per_layer_pruning_rate(
    ...
    """
    使用二分查找找到合适的 alpha，使得每层的平均剪枝率接近目标值
    ...
    # 这里不要再假设“第一层行数=每层专家数”，因为原始 CSV 可能缺专家行。
...
def save_results(
    ...
    output_json = os.path.join(
        output_dir, f"selected_experts_{strategy}_rate{rate_str}.json"
    )
    output_csv = os.path.join(
        output_dir, f"selection_stats_{strategy}_rate{rate_str}.csv"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="基于剪枝率的专家选择工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
策略说明:

  global    - 全局剪枝率：在所有层中总共保留 X% 的专家
  per_layer - 每层剪枝率：每层都保留 X% 的专家（推荐）

示例用法:

  # 每层保留 50% 专家（推荐）
  python select_experts_by_pruning_rate.py \\
      --input ../analysis/results/expert_shapley_values_all_layers.csv \\
      --pruning_rate 0.5 \\
      --strategy per_layer
  ...
  python select_experts_by_pruning_rate.py \\
    ...
    parser.add_argument(
        "--input", type=str, required=True, help="Shapley 值 CSV 文件路径"
    )
    parser.add_argument(
        "--pruning_rate",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="（可选）每层专家总数。若提供或可推断，将对每层缺失专家补齐 Shapley=0，避免 silent bug。",
    )
    parser.add_argument(
